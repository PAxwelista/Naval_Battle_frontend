import { gridIncludesValuesInPositions, changeGrid,createEmptyGrid ,replaceValuesOnGrid, subDragInfosToPositions} from "@/utils";
...
describe("gridIncludesValuesInPositions function", () => {
    ...
    it("should return true if some values are in positions",  () => {
        ...
        expect(gridIncludesValuesInPositions(grid,[{x:1,y:3},{x:2,y:3},{x:6,y:1}],["P","8"])).toBeTruthy()
    });
    it("should return false if none of values are in positions",  () => {
        ...
        expect(gridIncludesValuesInPositions(grid,[{x:1,y:3},{x:2,y:3},{x:6,y:1}],["P"])).toBeFalsy()
    });
    it("should return false if a position is out of grid",  () => {
        ...
        expect(gridIncludesValuesInPositions(grid,[{x:1,y:3},{x:2,y:3},{x:6,y:1},{x:8,y:8}],["P"])).toBeFalsy()
    });
});
...
describe("subDragInfosToPositions function", () => {
    it("should return positions from dragInfos",  () => {
        ...
        expect(positions).toEqual([{x:-1,y:1},{x:0,y:1},{x:1,y:1},{x:2,y:1}])
    });
    it("should return positions from dragInfos with a horizontal false",  () => {
        ...
        expect(positions).toEqual([{x:1,y:-1},{x:1,y:0},{x:1,y:1},{x:1,y:2}])
    });
